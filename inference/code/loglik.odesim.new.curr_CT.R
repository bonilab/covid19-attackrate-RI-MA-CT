...
loglik.odesim <- function(traj,
  ...
  # cat("odesim ", tot.size.ep.odesim, "\n")
  # cat("data ", tot.size.ep, " tot.size.percent.off ",tot.size.percent.off,"\n")
  ...
  if(min(report.rate, nb.disp.new.cases, nb.disp.new.hosp,s2.hosp,s2.icu,s2.vent) < 0 |
    ...
    ll.new <- NULL #NA #integer()
    ll.hosp <- NULL #NA #integer()
    ll.deaths <- NULL #NA #integer()
    ...
    if(lik.age & !active.surv){
      ...
      age.means <- apply(shifted.age.means, 2,
                         function(X){report.rate[t.idx.rr] * X[t.idx.delay]})
    ...
    if(lik.curr){ 
      ...
      ############################################################################
      ### 1. likelihood for current (not increments/conditional) hosp/icu/vent ###
      ############################################################################
      ...
      if(lik.hosp.curr){
        ...
        ll.hosp <- NULL #NA #integer(0)
      }
          
    } else { 
    
      ##############################################################################
      ### 2. likelihood for increments / conditional likelihood of hosp/icu/vent ###
      ##############################################################################
      
      if (lik.old){
        ### 2a) likelihood used in the original submission ###
        ...
        ### 2b) "new" likelihood, with dependence across current data streams ###
        ...
        if(lik.hosp.curr){
          ...
          if(loc == "RI"){ 
            ...
            for(ii in 1:nrow(mu.hiv)){
              llvals.hosp.icu.vent[ii] <- dmvnorm(c(delta.hosp[ii],
                                                    delta.icu[ii],
                                                    delta.vent[ii]),
                                                  mean = mu.hiv[ii,],
                                                  sigma = cov.hiv[,,ii],
              ...
              ll.icu <- NULL #NA #integer()
              ll.vent <- NULL #NA #integer()
    ...
    }## end likelihood for increments of hosp/icu/vent
          
          ## ## ## commented out by EMH 20210209
          ## ## ## code above should do what this did...
          ## if(lik.vent.curr){
          ##   no.na.idx=which(!is.na(delta.vent))
          ##   # Gaussian likelihood
