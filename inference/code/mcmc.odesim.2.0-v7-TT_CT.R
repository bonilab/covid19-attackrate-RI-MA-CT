  ...
  for(iter in 1:n.mcmc){
    ...
    while(beta.good < 1){
      ...
      traj.star <- try(traj.from.params(beta = beta.daily.star, 
                                        ...
                                        loc = loc,
                                        symp = symp.cur,
                                        mean.report.rate = mean(rr.daily)), silent = TRUE)
	...
	if(print.iter){cat(iter,".beta_mh1_",mh1,"__",sep="")}
	...
	if(print.iter){cat(iter,".ode_mh1_",mh1,"__",sep="")}
	...
	if(is.null(mh1) | length(mh1) == 0){
		...
		if(print.iter){cat(iter,".sfchoice_mh1_",mh1,"__",sep="")}
    ...
    ### setting up error catching - reject rr if odesim fails
    rr.good <- 0
    ...
    while(rr.good < 1){
      ...
      ### trajectory given rr.daily.star
      traj.star <- try(traj.from.params(beta = beta.daily, 
                                        params = ode.params,
                                        tf = end.day,
                                        introday = introday,
                                        ...
                                        loc = loc,
                                        symp = symp.cur,
                                        mean.report.rate = mean(rr.daily.star)), silent = TRUE)
    ...
    if (max(rr.daily.star, na.rm = T) < 1){    
      ...
      ## ### resample imputations, reevaluate likelihood for current rr.params
      ## llvals <- loglik.odesim(traj = traj.star,
      ##                         dp = data.processed,
      ##                         report.rate = rr.daily,
      ##                         nb.disp.params = lik.params,
      ##                         s2.hosp = s2.params[1],
	...
	if(print.iter){cat(iter,".rr_mh1_",mh1,"__",sep="")}
    ...
    if(!lik.hosp.curr & !lik.vent.curr & !lik.icu.curr){
      ...
      if(!is.null(llvals.star$ll.hosp) & length(llvals.star$ll.hosp) > 0){
        mh1 <- mh1 + llvals.star$ll.hosp
        #mh2 <- mh2 + llvals$ll.hosp
        s2.true[1] <- TRUE
      }
      if(!is.null(llvals$ll.hosp) & length(llvals$ll.hosp) > 0){
        mh2 <- mh2 + llvals$ll.hosp
      }	 
      
      if(!is.null(llvals.star$ll.icu) & length(llvals.star$ll.icu) > 0){
        mh1 <- mh1 + llvals.star$ll.icu
        #mh2 <- mh2 + llvals$ll.icu
        s2.true[2] <- TRUE
      }
      if(!is.null(llvals$ll.icu) & length(llvals$ll.icu) > 0){
        mh2 <- mh2 + llvals$ll.icu
      } 
      
      if(!is.null(llvals.star$ll.vent) & length(llvals.star$ll.vent) > 0){
        mh1 <- mh1 + llvals.star$ll.vent
        #mh2 <- mh2 + llvals$ll.vent
        s2.true[3] <- TRUE
      } 
      if(!is.null(llvals$ll.vent) & length(llvals$ll.vent) > 0){
        mh2 <- mh2 + llvals$ll.vent
      }
	...
	if(print.iter){cat(iter,".s2_mh1_",mh1,"__",sep="")}
...
### evaluate logliklihood with lik.params.star
		...
		if(print.iter){cat(iter,".lik_mh1_",mh1,"__",sep="")}
